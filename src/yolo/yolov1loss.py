...
class Yolov1Loss(nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        noo_pred_mask = torch.zeros(noo_pred.size(), dtype=torch.bool).to(DEVICE)
        noo_pred_mask[:, 4] = True
        noo_pred_mask[:, 9] = True
        noo_pred_c = noo_pred[noo_pred_mask]
        noo_target_c = noo_target[noo_pred_mask]
        nooobj_loss = F.mse_loss(noo_pred_c, noo_target_c, reduction='sum')

        # compute contain obj loss
        coo_response_mask = torch.zeros(box_target.size(), dtype=torch.bool).to(DEVICE)
        coo_not_response_mask = torch.zeros(box_target.size(), dtype=torch.bool).to(DEVICE)
